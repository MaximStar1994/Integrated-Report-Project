...
class SideBar extends React.Component {
    ...
    GetMenu = () => {
        var options = ["Support Report", "Real Time Alarm"]
        const items = options.map(key => 
            <Row className="sidebarheader clickable"  key = {key} style={{padding : "15px 0"}}>
                <Col style={{color : "white"}} onClick={() => {
                    if (key === "Real Time Alarm") {
                        this.props.history.push('/insight/realtimealarm')
                    } else if (key === "Support Report") {
                        this.props.history.push('/insight/supportreport')
                    }
                }}>
                   {key}
                </Col>
