...
class PostgreSQLInterface {
    constructor (mconfig = null) {
        if (mconfig == null) {
            this.pool = new Pool(config.kstConfig.dbConfig)
        } else {
    ...
    async InsertRow(datarow, mapping, tablename) {
        ...
        Object.keys(mapping).forEach(key => {
            newrow = helperWithoutApi.updateObjForPG(datarow[key], `${mapping[key].col}`, newrow)
        })
        ...
        await Promise.all(
            Object.keys(config.sqlTables).map(async (key) => {
                if (
                    ...
                    await this.PerformQueryPromise(`DELETE FROM ${config.sqlTables[key]}`, [])
                    return key;
                } else {
                    return key;
                }
    ...
    PerformBatchInsert(table, newDataArray, callback) {
        ...
        if (newDataArray instanceof Array && newDataArray.length > 0 && Object.keys(newDataArray[0]).length > 0) {
...
module.exports = { interfaceObj : new PostgreSQLInterface(), constructor : PostgreSQLInterface};